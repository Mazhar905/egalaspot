...
// Example dropdown component for login and registration
const Dropdown = ({ isOpen }) => {
	// This can be expanded with actual login and registration forms
	return isOpen ? (
...
function Header() {
	...
	useEffect(() => {
		...
		const delaySearch = setTimeout(() => {
			if (searchQuery.trim().length > 0) {
				...
				const results = mainMenu.filter((item) => item.name.toLowerCase().includes(searchQuery.toLowerCase()));
	...
	// Function to toggle the dropdown for profile
			...
			<header className="shadow-md bg-white tracking-wide relative z-30">
				<section className="flex items-center flex-wrap lg:justify-center gap-4 py-3 sm:px-2 px-4 border-gray-200 border-b min-h-[75px]">
							...
							<div className="absolute mt-1 w-full bg-white rounded-md shadow-lg z-10">
								{searchResults.map((result, index) => (
									<Link key={index} href={result.link}>
										<div className="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">{result.name}</div>
									</Link>
								))}
							</div>
						)}
					...
					<div className="lg:absolute lg:right-10 flex items-center ml-auto space-x-8">
						...
						<div className="relative -mb-1">
							<Link href={"/profile"} className="inline-block cursor-pointer border-gray-300">
				...
				<div className="flex flex-wrap justify-center lg:px-10 lg:py-3 relative">
					<div
						...
						className={`${
							isMenuOpen ? "" : "hidden"
						} lg:flex max-lg:before:fixed max-lg:before:bg-black max-lg:before:opacity-40 max-lg:before:inset-0 max-lg:before:z-50`}
					>
												...
												{menuItem.subMenu.map((subMenuItem, subIndex) => (
													<li key={subIndex} className="ps-3 yborder-b py-3">
														<Link
															...
															className="hover:text-[#F04725] hover:fill-[#F04725] text-gray-600 font-semibold text-[15px] block"
														>
															{subMenuItem.name}
														</Link>
													</li>
